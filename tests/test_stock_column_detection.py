"""Regression tests for the a regional food distributor "all healthy but no stock" incident.
...
# The a regional food distributor sheet, exactly as _sanitize_name stores it. Column order
# matters: qty_sold comes BEFORE current_system_balance, which is what made
# first-match-wins pick the wrong one.
DISTRIBUTOR_COLS = ["category", "item_name", "supplier", "sales_pre_tax",
                 "qty_sold", "unit", "avg_qty_month", "current_system_balance"]

_check("a regional food distributor sheet: stock = current_system_balance, never qty_sold",
       _pick_stock_column(DISTRIBUTOR_COLS) == "current_system_balance",
       detail=str(_pick_stock_column(DISTRIBUTOR_COLS)))
