...
try:
    ...
    emails._send_run_failure_alert("Cool Link", 7, "blank", "", "https://berthcast.com")
    ...
    _check("blank alert names the client + reason",
           "Cool Link" in _sent[0][0] and "BLANK" in _sent[0][2], detail=str(_sent[0]))

    _sent.clear()
    os.environ.pop("ALERT_EMAIL")
    emails._send_run_failure_alert("Cool Link", 7, "failed", "kaboom", "https://berthcast.com")
...
_mk_user("buyer@coollink.com", "Cool Link")
_mk_user("nobody@dormant.com", "DormantCo")     # logs in, never runs anything

# Cool Link: one healthy, one blank, one failed.
_mk_run("Cool Link", "complete",
        inventory=[{"item": "Frozen Peas", "status": "OK"}, {"item": "Rice", "status": "LOW"}],
        recs=[{"item": "Rice", "qty": 100}])
_mk_run("Cool Link", "complete", inventory=[], recs=[])      # blank
_mk_run("Cool Link", "failed")
...
_check("page shows the client org", "Cool Link" in body)
...
client.get("/logout")
client.post("/login", data={"email": "buyer@coollink.com", "password": "password123"})
