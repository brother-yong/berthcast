"""Proof for the clearer purchase-order outputs (Cool Link feedback).

Cool Link staff couldn't read the printed order: the quantity column was ambiguous
...
db.execute("INSERT INTO users (email, password_hash, org_name, model, tier) VALUES (?,?,?,?,?)",
           ("buyer@coollink.com", generate_password_hash("x"), "Cool Link",
