...
db.execute("INSERT INTO users (email, password_hash, org_name, model) VALUES (?,?,?,?)",
           ("staff@coollink.com.sg", generate_password_hash("old-password-1"), "Cool Link", "claude-sonnet-4-6"))
user = db.query("SELECT id FROM users WHERE email=?", ("staff@coollink.com.sg",))[0]["id"]
