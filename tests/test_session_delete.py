...
# ── Seed: two analyses for a regional food distributor, one for another org ────────────────────
db.execute("INSERT INTO users (email, password_hash, org_name, model, tier) VALUES (?,?,?,?,?)",
           ("buyer@example.com", generate_password_hash("x"), "a regional food distributor", "claude-sonnet-4-6", "enterprise"))
uid = db.query("SELECT id FROM users WHERE email=?", ("buyer@example.com",))[0]["id"]

A = db.execute("INSERT INTO upload_sessions (user_id, org_name, status) VALUES (?,?,?)", (uid, "a regional food distributor", "complete"))
B = db.execute("INSERT INTO upload_sessions (user_id, org_name, status) VALUES (?,?,?)", (uid, "a regional food distributor", "complete"))
