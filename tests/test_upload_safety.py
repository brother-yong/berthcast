...
db.execute("INSERT INTO users (email, password_hash, org_name, model, role) VALUES (?,?,?,?,?)",
           ("analyst@coollink.com", generate_password_hash("x"), "Cool Link",
            "claude-sonnet-4-6", "admin"))
uid = db.query("SELECT id FROM users WHERE email=?", ("analyst@coollink.com",))[0]["id"]

client = appmod.app.test_client()
with client.session_transaction() as s:
    s["user_id"]  = uid
    s["email"]    = "analyst@coollink.com"
