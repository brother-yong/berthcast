...
email, err = validators.validate_email_change("jo hn@example.com.sg", TARGET, _free)
...
email, err = validators.validate_email_change("john@example.", TARGET, _free)
_check("domain ending with dot is rejected", email is None and err)

# --- uniqueness ---------------------------------------------------------------
email, err = validators.validate_email_change("taken@example.com.sg", TARGET, _taken_by(9))
_check("email owned by another account is rejected", email is None and err,
       detail=f"{email!r}, {err!r}")

email, err = validators.validate_email_change("mine@example.com.sg", TARGET, _taken_by(TARGET))
_check("re-setting the account's own email is allowed",
       email == "mine@example.com.sg" and err is None, detail=f"{email!r}, {err!r}")

# --- success + normalisation --------------------------------------------------
email, err = validators.validate_email_change("john@example.com.sg", TARGET, _free)
