...
Proves the feature does what it says once a human has confirmed a pairing:
  ...
  3. the confirm screen renders and lists the uploaded tender rows
  4. saving stores the CANONICAL item spelling, not what was typed
  5. the results page renders base + tender, in the gold class
  6. the printed sheet carries the total and a "+ N tender" sub-line
  7. the CSV gains a Tender Add-On column and exports the total
...
# ── 3. The confirm screen renders the uploaded rows ──────────────────────────

r = _upload_tenders(alpha, TENDER_CSV, "alpha_tenders.csv")
_check("the tender sheet imported", b"Imported 2 tender rows" in r.data)

r = alpha.get("/tenders/match")
html = r.get_data(as_text=True)
_check("the match screen renders", r.status_code == 200, detail=str(r.status_code))
_check("it shows the tender text as it appears on their sheet", T_SAUCE in html)
_check("it offers the matching item as a candidate", SAUCE in html)
_check("it states that nothing is applied until decided",
       "Nothing is added to your order quantities until you decide it here." in html)
_check("nothing is added before a decision", appmod._tender_addon_map("OrgAlpha") == {})


# ── 4. Saving stores the canonical spelling, not what was typed ──────────────

r = alpha.post("/tenders/match", data={
    # Typed by hand into the free-text box, in the wrong case and with a
    # doubled space: the stored name must still be the org's own spelling.
    "match__" + K_SAUCE: "__other__",
    "other__" + K_SAUCE: "padimas oyster sauce  500ml",
    "match__" + K_COD:   COD,
}, follow_redirects=True)
_check("saving redirects back to the screen", r.status_code == 200, detail=str(r.status_code))

saved = {m["tender_key"]: m for m in db.get_tender_matches("OrgAlpha")}
_check("both decisions were stored", len(saved) == 2, detail=str(sorted(saved)))
...
_check("the stored key is the canonical item's key",
       ...
       detail=str(saved.get(K_SAUCE)))
_check("the confirmed pairing now carries a monthly figure",
...
_check("the confirmation still applies after a re-upload",
       appmod._tender_addon_map("OrgAlpha").get(normalise_match_key(SAUCE), {}).get("qty") == 100.0,
       detail=str(appmod._tender_addon_map("OrgAlpha")))
