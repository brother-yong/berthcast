...
db.execute("INSERT INTO users (email, password_hash, org_name, model) VALUES (?,?,?,?)",
           ("staff@example.com", generate_password_hash("x"), "a regional food distributor", "claude-sonnet-4-6"))
db.execute("INSERT INTO users (email, password_hash, org_name, model) VALUES (?,?,?,?)",
           ("other@example.com.sg", generate_password_hash("x"), "a regional food distributor", "claude-sonnet-4-6"))
userA = db.query("SELECT id FROM users WHERE email=?", ("staff@example.com",))[0]["id"]
userB = db.query("SELECT id FROM users WHERE email=?", ("other@example.com.sg",))[0]["id"]
...
r = client.post("/admin", data={"action": "change_email", "user_id": str(userA),
                                "new_email": "other@example.com.sg"})
...
_check("userA email unchanged after duplicate attempt",
       _email_of(userA) == "staff.real@example.com.sg", detail=_email_of(userA))
_check("userB email untouched", _email_of(userB) == "other@example.com.sg", detail=_email_of(userB))
